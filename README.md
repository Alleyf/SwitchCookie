...
| 场景 | 截图 |
|------|------|
| 弹出窗 · 账号列表 | ![账号列表](docs/screenshots/popup-accounts.png) |
| 弹出窗 · Cookie / 存储编辑器 | ![编辑器](docs/screenshots/popup-editor.png) |
| 标签隔离绑定提示条 | ![绑定提示](docs/screenshots/bind-bar.png) |

> 截图需在扩展加载到浏览器后，于对应界面用系统截图工具捕获，存放于 `docs/screenshots/` 目录。
...
| 页面用 iframe 沙箱或 `Object.freeze(window)` 冻结 localStorage 描述符 | 该 frame 内隔离失效 | defineProperty 会静默失败 |
